...
def format_stream_details(filename: str, quality: str, size: str, is_split: bool = False, split_count: int = 0) -> tuple[str, str]:
    """Format stream name and title for display in Stremio.
    ...
    Split file:
        name  → "Telegram | 1080p | WEB-DL | Split Video"
        title → "filename.mkv [Split x2]\n\n📦 5.83GB"
    """
    try:
    ...
    if is_split:
        ...
        if quality_type:
            ...
            stream_name = f"Telegram | {resolution} | Split Video"

        # Strip .partXXX suffix from filename for clean display
        clean_filename = strip_part_suffix(filename)
...
async def get_manifest(token: str, token_data: dict = Depends(verify_token)):
    ...
    return {
        ...
        "name": addon_name,
        "logo": "https://i.postimg.cc/9QHmQZZ4/Telegram-logo.jpg",
...
async def get_catalog(token: str, media_type: str, id: str, extra: Optional[str] = None, token_data: dict = Depends(verify_token)):
    ...
    try:
        ...
        elif id in ("tg_movies", "tg_series") and search_query:
    ...
    if media_details and "telegram" in media_details:
        for quality in media_details.get("telegram", []):
            if quality.get("id"):
                ...
                split_count = len(parts) if parts else 0

                stream_name, stream_title = format_stream_details(
                    filename, quality_str, size, is_split=is_split, split_count=split_count
                )
